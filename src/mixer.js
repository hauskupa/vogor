...
// Shared mixer binding for other experiences (e.g. cassette deck).
export function bindMixerTrackControls({ root = document, tracks = [] } = {}) {
  const disposers = [];
  const rows = Array.from(root.querySelectorAll("[data-mixer-track]"));
  ...
  function setSoloState(activeTrackId = null) {
    tracks.forEach((track) => {
      if (!track || !track.audio) return;
      const isSoloActive = Boolean(activeTrackId);
      const shouldMute = isSoloActive && track.id !== activeTrackId;
      track.audio.muted = shouldMute || track._manualMute;
    });
  }
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const track =
      tracks.find((t) => String(t.id) === String(rowTrackId)) ||
      tracks[rowIndex];

    if (!track || !track.audio) return;

    const volumeControl = row.querySelector("[data-volume-control]");
    const panControl = row.querySelector("[data-pan-control]");
    const muteButton = row.querySelector("[data-mute-button]");
    const soloButton = row.querySelector("[data-solo-button]");
    const nameEl = row.querySelector("[data-track-name]");

    if (nameEl) nameEl.textContent = track.name || track.id || "Stem";

    if (volumeControl) {
      if (!volumeControl.value) volumeControl.value = String(track.audio.volume);
      addListener(volumeControl, "input", (e) => {
        track.audio.volume = parseFloat(e.target.value || "1");
      });
    }

    if (panControl && track.panner) {
      addListener(panControl, "input", (e) => {
        track.panner.pan.value = parseFloat(e.target.value || "0");
      });
    }

    if (muteButton) {
      addListener(muteButton, "click", () => {
        track._manualMute = !track._manualMute;
        muteButton.classList.toggle("mute-active", track._manualMute);
        setSoloState(
          tracks.find((t) => t._soloActive)?.id || null
        );
      });
    }

    if (soloButton) {
      addListener(soloButton, "click", () => {
        const shouldActivate = !track._soloActive;
        tracks.forEach((t) => {
          t._soloActive = false;
        });
        track._soloActive = shouldActivate;

        rows.forEach((r) => {
          const b = r.querySelector("[data-solo-button]");
          if (b) b.classList.remove("solo-active");
        });
        soloButton.classList.toggle("solo-active", shouldActivate);
        setSoloState(shouldActivate ? track.id : null);
      });
    }
  });
